...
//   noTitle: { scope: '[data-test-no-title]' },
//
...
module('Acceptance | breadcrumbs', function (hooks) {
  ...
  module('Breadcrumbs properties', function () {
    test('visiting', async function (assert) {
      ...
      assert.equal(Object.keys(crumbs[1].routeParams).length, 0, "route Params");
      assert.equal(Object.keys(crumbs[1].routeQueryParams).length, 0, "routeQueryParams");
    });
  });
